// Copyright (c) 2009-2012 CommunityCoin Developers
...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <communitycoinprivkey> [label] [rescan=true]\n"
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by CommunityCoin %s (%s)\n", CLIENT_BUILD.c_str(), CLIENT_DATE.c_str());
